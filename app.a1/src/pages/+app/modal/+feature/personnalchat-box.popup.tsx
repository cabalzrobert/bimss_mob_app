...
export default class PersonnalChatBoxPopUp extends React.Component<{ modal: Function }> implements OnDidBackdrop {
    state: any = {}
    componentWillMount = () => {
        app.component(this);
        const { prop } = this;;
    ...
    private async setUpChat() {
        ...
        //Object.rcopy(this.prop, personnalchatBox.info);
        Object.rcopy(this.prop, personnalchatBox.info, operatorDetails);
        console.log('setUpChat personnalchatbox');
        console.log([this.prop, this.prop.ChatKey, this.prop.RequestID, this.prop.DisplayName]);
        if (this.prop.IsLoaded) {
            this.list = personnalchatBox.list;
        ...
        return this.performRequestDetailDelay({ IsReset: true, RequestID: this.prop.RequestID }, () => setTimeout(() => this.hScrollMessages(), 125));
    }
    private receivedChat(data: any) {
        ...
        if (data.type == 'personal') {
            ...
            if (!found) {
                this.list.unshift(personnalchatBox.details(message));
                this.prop.IsEmpty = (this.list.length < 1);
            }
            this.setState({ list: this.list, prop: this.prop });
        }
    ...
    hSend = () => {
        ...
        const copy = personnalchatBox.details(Object.rcopy(input));
    ...
    private performSubmit(input: any, message: any) {
        ...
        rest.post('/app/chat/t/' + this.prop.ChatKey, input).subscribe(async (res: any) => {
            if (res.Status != 'error') {
                ...
                if (!(index < 0)) {
                    this.list.splice(index, 1);
                    personnalchatBox.details(Object.rcopy(found, res));
                }
                this.setState({ list: this.list });
            }
        }, (err: any) => {
        ...
        this.subs.s1 = rest.post('/app/chat/r/' + filter.RequestID, filter).subscribe(async (res: any) => {
            ...
            if (res.Status != 'error') {
                if (personnalchatBox.info.IsLoaded) {
                    ...
                    delete res.Messages;
                    Object.rcopy(this.prop, personnalchatBox.setInfo(res));
                    this.list = personnalchatBox.setList(messages.map((o: any) => personnalchatBox.details(o)));
                }
                this.prop.IsEmpty = (this.list.length < 1);
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s2 = rest.post('/app/chat/t/' + this.prop.ChatKey + '/' + id, filter).subscribe(async (res: any) => {
            ...
            if (res.Status != 'error') {
                ...
                if (lastID == id) {
                    res.forEach((o: any) => this.list.push(personnalchatBox.details(o)));
                    this.prop.IsEmpty = (this.list.length < 1);
                }
                if (callback != null) callback();
                this.setState({ list: this.list, prop: this.prop });
                ...
                <div className="modal-container" style={styles('height:675px;max-height:95%;background-color:white;border-radius:15px;')}>
                    <div style={styles('padding-top:5px')}>
                        <div className="row m-0 header">
                            <div className="col-10"><b>Operator Chat</b></div>
    ...
    static modal = async () => {
        ...
        var stack = await Stack.push(<>
            <Modal className="modal-adjustment no-bg w-100p h-100p" ref={(ref) => modal = ref} content={<PersonnalChatBoxPopUp modal={() => modal} />} />
        </>);
