import { IonButton, IonFooter, IonInput, IonItem, IonLabel } from '@ionic/react';
...
export default class RequestBrgyClearancePopUp extends React.Component<{ modal: Function, Subscriber: any }> implements OnDidBackdrop {
    state: any = {};
    filter: any = {};
    componentWillMount = () => {
        app.component(this);
    ...
    render() {
        ...
        return (<>
            <div className="modal-container">
                <div className="sm-option-bg" style={styles('margin-bottom:2%;')}>

                    <div className="row m-0 bg-top option-container" style={styles('overflow:auto;')}>
                        <div className="col-12 p-0 row m-0 options" style={styles('')}>
                            <div className="col-12" style={styles('text-align:center;height:auto;font-size:14pt;font-weight:bold')}>Control #: {subscriber.ControlNo} {subscriber.NextFilter}</div>
                        </div>
                        {/* <div>
                            <IonLabel style={styles('')}>Control #: {subscriber.ControlNo} {subscriber.NextFilter}</IonLabel>
                        </div> */}
                        <div className="col-12 p-0 row m-0 options">
                            <div className="col-5 p-0 row m-0 options">
                                <div className="col-12" style={styles('text-align:right;height:auto;font-size:12pt;font-weight:bold')}>Type</div>
                            </div>
                            <div className="col-7 p-0 row m-0 options">
                                <div className="col-12" style={styles('text-align:left;height:auto;font-size:12pt')}>{subscriber.TypeofClearanceNM}</div>
                            </div>
                        </div>
                        <div className="col-12 p-0 row m-0 options">
                            <div className="col-5 p-0 row m-0 options">
                                <div className="col-12" style={styles('text-align:right;height:auto;font-size:12pt;font-weight:bold')}>Purpose</div>
                            </div>
                            <div className="col-7 p-0 row m-0 options">
                                <div className="col-12" style={styles('text-align:left;height:auto;font-size:12pt')}>{subscriber.PurposeNM
                                }</div>
                            </div>
                        </div>
            ...
            {/* <div style={styles('text-align:center;')}>
                <IonLabel style={styles('font-size:14pt;font-weight:bold;')}>Control #: {subscriber.ControlNo} {subscriber.NextFilter}</IonLabel>
            </div>
            <div className="col-12 p-0 row m-0 options">
                <div className="col-5 p-0 row m-0 options">
                    <div className="col-12" style={styles('text-align:right;height:auto;font-size:12pt;font-weight:bold')}>Type</div>
            ...
            <div className="col-12 p-0 row m-0 options" hidden={(subscriber.StatusRequest == '1' || subscriber.StatusRequest == '0') ? true : false}>
                ...
                <div className="col-7 p-0 row m-0 options">
                    <div className="col-12" style={styles('text-align:left;height:auto;font-size:12px')}>{(subscriber.Release) ? "Release" : "For Releasing"}</div>
                </div>
            </div>
            <div className="col-12 p-0 row m-0 options" hidden={(subscriber.StatusRequest == '3') ? false : true}>
                <div className="col-5 p-0 row m-0 options">
    ...
    static modal = async (subscriber: any = {}) => {
        ...
        var stack = await Stack.push(<>
            <Modal className="modal-adjustment width-350 no-bg mwf" ref={(ref) => modal = ref} content={<RequestBrgyClearancePopUp modal={() => modal} Subscriber={subscriber} />} />
        </>);
