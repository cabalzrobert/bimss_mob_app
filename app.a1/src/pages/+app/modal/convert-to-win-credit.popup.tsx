...
export default class ConvertToWinCreditPopUp extends React.Component<{ modal: Function }> implements OnDidBackdrop {
    state: any = { u: {} }
    componentWillMount = () => {
        app.component(this);
    ...
    private performSubmit() {
        rest.post('credit/to/wc', this.input).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
    ...
    private performLocalProcess() {
        additionalWinningBalance(+this.input.Amount);
            ...
            <div className="modal-container">
                <div style={styles('padding-top:5px')}>
                    <div className="row m-0 header">
                        <div className="col-10">Convert to Win Credit</div>
    ...
    static modal = async () => {
        ...
        var stack = await Stack.push(<>
            <Modal className="modal-adjustment" ref={(ref) => modal = ref} content={<ConvertToWinCreditPopUp modal={() => modal} />} />
        </>);
