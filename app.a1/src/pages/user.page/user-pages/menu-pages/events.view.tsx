import { IonButton, IonButtons, IonCard, IonCardContent, IonContent, IonFab, IonFabButton, IonHeader, IonIcon, IonInput, IonItem, IonList, IonMenuButton, IonRefresher, IonRefresherContent, IonRippleEffect, IonSelect, IonSelectOption, IonSpinner, IonTitle, IonToolbar } from '@ionic/react';
...
export default class EventsView extends React.Component implements OnPagerFocus {
...
export class ViewHolder extends React.Component {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('a/events', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
    ...
    async popUpCustomerActionPopUp(subscriber: any, filter: any) {
        const modal = await EventActionPopUp.modal(subscriber, filter);
        await modal.present();
                ...
                <div className="layout-horizontal">
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            <div style={styles('font-size: 14pt;font-weight: bold')}>{item.Title}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Description: {item.Description}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Date/Time: {item.EventDateTime}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Location: {item.Location}</div>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                ...
                                <IonButton color='success' size='small' hidden={accntype.isMember} onClick={onUpdateEvent}>Update</IonButton>
                            </div>
                        </div>
                    </div>
                </div>
