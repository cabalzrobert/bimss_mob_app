...
export default class EducationBackgroundAppHistory extends React.Component implements OnPagerFocus { //OnSwapFocus, 
...
export class ViewHolder extends React.Component {
    ...
    onPageFocus(data: any) {
        ...
        if (!data.IsUpdate) {
            ...
            Object.rcopy(this.selectedItem, this.listDetails(data.item));
    ...
    hItem = async (item: any) => {
        ...
        this.pager.next(AccountEducBackgroundView, ({ instance }: { instance: AccountEducBackgroundView }) => {
            instance?.setType(item.Type, item.type).setForm(item);
        });
    }
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('educ/loadattainment', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            ...
            if (res.Status != 'error') {
                ...
                console.log('this.prop.IsEmpty performRequest');
                        ...
                        <div style={styles('padding:5px')}>
                            <div className="row m-0">
                                <div className="col-12 p-0">
                                    <div style={styles('font-size:12px;color:#000000;')}>Enter any keyword to search and hit search button {prop.IsEmpty}</div>
                                </div>
                            </div>
                            <div className="row m-0">
                                    ...
                                    <IonItemSearch lines="none" className="input-error bg-adjust" style={styles('--highlight-background:unset;--min-height:40px')}>
                                        <Input ion="popup" node={(handle) => <>
                                            <IonInput className="font-bold br-0" style={styles('--padding-bottom:0px;--padding-top:0px;color:#fefed1')}
                                                type="text" placeholder="Search Educational Attainment" value={filter.Search} {...handle({ onChange: (ev) => this.hFState(ev, 'Search') })} />
                                        </>} />
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            ...
                            <div style={styles('font-size: 14pt;font-weight: bold')}>{item.EducLevel}</div>
                            <div style={styles('font-size: 14pt;font-weight: bold')}>{item.School}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')}>{item.SchoolAddress} {item.NextFilter}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')}>{item.SchoolYear}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')} hidden={(item.EducLevel == 'Senior Highschool' || item.EducLevel == 'Vocational' || item.EducLevel == 'College') ? false : true}> {/*(item.EducLevel != 'Senior Highschool') ? 'Course: ' : 'Strand: '*/} {item.Course}</div>

                        </div>
                        <div className="horizontal" style={styles('justify-content: left;')}>
                                {/* <div className="btn-update" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onUpdateIssuesConcern}>Update</div>&nbsp; */}
                                <IonButton size="small" onClick={onUpdateIssuesConcern}>
                                    Update
                                </IonButton>
