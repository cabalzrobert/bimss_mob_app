...
import RequestBrgyOtherDocumentView from './brgyotherdocument.new.view';

const { Object }: any = window;

export default class RequestBarangayOtherDocumentAppHistory extends React.Component implements OnPagerFocus {
    ...
    //};
    onPagerFocus = (data: any) => (RequestBarangayOtherDocumentAppHistory.activeTab && RequestBarangayOtherDocumentAppHistory.activeTab.onPagerFocus(data));
...
class PowerApTabsView extends React.Component {
    ...
    onSwapChange = () => {
        return timeout(() => {
            RequestBarangayOtherDocumentAppHistory.activeTab = this.swapper.currentComponent.instance;
        });
...
class PageFilterOpenView extends React.Component<{ status?: (undefined | '0' | '1' | '2' | '3') }> {
    ...
    hItem = async (item: any) => {
        ...
        //console.log(item);
        this.pager.next(RequestBrgyOtherDocumentView, ({ instance }: { instance: RequestBrgyOtherDocumentView }) => {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('otherdocument/history', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            ...
            if (res.Status != 'error') {
                ...
                if (filter.IsReset) this.list = res.lgldoctrans.map((o: any) => Customer(o));
    ...
    hAddItem = (ev: any) => {
        ...
        this.filter.isAdd = true;
        this.filter.isEdit = false;
        this.pager.next(RequestBrgyOtherDocumentView, ({ instance }: { instance: RequestBrgyOtherDocumentView }) => instance?.setType(this.filter.isAdd, '', ''));
...
class PageFilterReceivedView extends React.Component<{ status?: (undefined | '0' | '1' | '2' | '3') }> {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        if (!this.subs) return;
        if (this.subs.s1) this.subs.s1.unsubscribe();
        this.subs.s1 = rest.post('reqotherdocument', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            this.prop.IsFiltering = false;
            console.log(res);
            if (res.Status != 'error') {
                if (filter.IsReset) this.list = res.lgldoctrans.map((o: any) => Customer(o));
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                            ...
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Type: {item.TemplateTypeNM}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Document: {item.TemplateNM}</div>
