import { IonButton, IonContent, IonIcon, IonInput, IonItem, IonLabel, IonTextarea, IonSelect, IonSelectOption, IonList, IonRefresher, IonRefresherContent, IonRippleEffect, IonSearchbar, IonSpinner, IonDatetime, IonTitle, IonHeader } from '@ionic/react';
...
export default class AccountEmploymentHistoryView extends React.Component {
...
export class ViewHolder extends React.Component {
    ...
    willMount = (didMount = true) => {
        ...
        opts.dtConfig = { MinYear: '1910', MaxYear: today.getFullYear() + 5, MinMonth: ("0" + (today.getMonth() + 1)).slice(-2), MinDate: today.getFullYear() + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-' + ("0" + (today.getDate())).slice(-2), MaxDate: (today.getFullYear() + 1) + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-' + ("0" + (today.getDate())).slice(-2), MinTime: '00:00', MaxTime: '12:00' }
    ...
    setForm(input: any) {
        ...
        this.prop.IsUpdate = input.isEdit;
        this.input.IsUpdate = input.isEdit;
        this.input.isAdd = input.isAdd;
    ...
    setType(type: any) {
        console.log('type');
        console.log(type);
        this.input.isAdd = false;
        this.input.IsUpdate = this.input.isEdit;
        this.setState({ input: this.input });
        console.log('Set Type this.input');
        console.log(this.input);
        return this;
    }
    ...
    private performSubmit() {
        ...
        rest.post('employment/update', this.input).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.educattainment);
                console.log('performSubmit');
                console.log(item);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({ item, component: AccountEmploymentHistoryView, IsUpdate: !!this.input.IsUpdate, }));
            }
    ...
    private isValidEntries(): boolean {
        ...
        if (!this.input.Company) {
            this.msg.Subject = 'Please enter Company'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
        if (!this.input.CompanyAddress) {
            this.msg.Subject = 'Please enter Company Address'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
        if (!this.input.RenderedFrom) {
            this.msg.Subject = 'Please enter Redered From Date'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
            ...
            <Layout full>
                <Layout>
                    {/* <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-7 p-0 vertical" style={styles('align-items: end;color: black;font: message-box;')}><div><TextFit text="Employment History" /></div></div>
                    </div> */}
                        ...
                        <div className="row m-0 bootstrap-form old">
                            <div className='col-12 p-0 form-container' style={styles('width: 100%;')}>
                                <form className={classNames('needs-validation', { 'form-invalid': !input.IsValid })} noValidate onSubmit={(ev) => ev.preventDefault()} style={styles('padding:10px;')}>
                                    <div className={classNames({ 'input-invalid': !!msg.Company })} style={styles('position:relative;margin-top:7.5px;')}>
                                            <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Company</IonLabel>
                                        <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            <Input ion node={(handle) => <>
                                                <IonInput className="font-bold br-0"
                                                    type="text"
                                                    value={this.input.Company} {...handle({ onChange: (ev) => this.hState(ev, 'Company') })} />
                                            </>} />
                                        </IonItem>
                                        <div className="invalid-tooltip">{msg.Company}</div>
                                    </div>
                                    <div className={classNames({ 'input-invalid': !!msg.CompanyAddress })} style={styles('position:relative;margin-top:7.5px;')}>
                                            <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Address</IonLabel>
                                        <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            <Input ion node={(handle) => <>
                                                <IonInput className="font-bold br-0"
                                                    type="text"
                                                    value={this.input.CompanyAddress} {...handle({ onChange: (ev) => this.hState(ev, 'CompanyAddress') })} />
                                            </>} />
                                        </IonItem>
                                        <div className="invalid-tooltip">{msg.CompanyAddress}</div>
                                    </div>

                                    <div className="row m-0">
                                        <div className="col-6 p-0">
                                            <div className={classNames({ 'input-invalid': !!msg.RenderedFrom })} style={styles('position:relative;margin-top:7.5px;')}>
                                                    <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Rendered From</IonLabel>
                                                <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px;maring-right:1.25px;')}>
                                                    <Input ion node={(handle) => <>
                                                        ...
                                                        <IonDatetime pickerFormat="MMMM DD YYYY" displayFormat="MMM DD, YYYY" style={styles('--padding-bottom:5px;height:50px')}
                                                            ...
                                                            value={input.RenderedFrom} {...handle({ onChange: (ev) => this.hState(ev, 'RenderedFrom') })} />
                                                    </>} />
                                                </IonItem>
                                                <div className="invalid-tooltip">{msg.RenderedFrom}</div>
                                            </div>
                                        </div>
                                        <div className="col-6 p-0">
                                            <div className={classNames({ 'input-invalid': !!msg.RenderedTo })} style={styles('position:relative;margin-top:7.5px;')}>
                                                    <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Rendered To</IonLabel>
                                                <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px;margin-left:1.25px;')}>
                                                    <Input ion node={(handle) => <>
                                                        <IonLabel hidden position="stacked"></IonLabel>
                                                        <IonInput hidden />
                                                        <IonDatetime pickerFormat="MMMM DD YYYY" displayFormat="MMM DD, YYYY" style={styles('--padding-bottom:5px;height:50px')}
                                                            min={this.opts.dtConfig.MinYear} max={this.opts.dtConfig.MaxYear}
                                                            value={input.RenderedTo} {...handle({ onChange: (ev) => this.hState(ev, 'RenderedTo') })} />
                                                    </>} />
                                                </IonItem>
                                                <div className="invalid-tooltip">{msg.RenderedTo}</div>
                                            </div>
                                        </div>
                                    </div>
