...
import ValidGovernmentIDView from './account.validgovernmentid.new.view';
...
export default class ValidGorvernmentIDAppHistory extends React.Component implements OnPagerFocus { //OnSwapFocus, 
...
export class ViewHolder extends React.Component {
    ...
    hAddItem = (ev: any) => {
        ...
        this.pager.next(ValidGovernmentIDView, ({ instance }: { instance: ValidGovernmentIDView }) => instance?.setType(this.filter.isAdd));
    }
    // hMemberOpt = (ev: any) => {
    //     var type = (ev.detail || mtObj).value;
    //     if (!type) return;
    ...
    hItem = async (item: any) => {
        ...
        this.pager.next(ValidGovernmentIDView, ({ instance }: { instance: ValidGovernmentIDView }) => {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('validid/history', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
                ...
                <Layout auto>
                    ...
                    <IonContent scrollY>
                        ...
                        {/* <IonFab vertical="bottom" horizontal="end" slot="fixed" style={styles('margin-bottom:3vw;margin-right:2vw;bottom:5%;')}>
                            <IonFabButton style={styles('height:75px;width:75px;')} onClick={this.hAddItem}>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            <div style={styles('font-size: 14pt;font-weight:bold;margin-top: -2.5px;margin-bottom:2.5px;')}>{item.GovernmentID_NM}</div>
                            <div style={styles('font-size: 12pt;font-weight:bold;margin-top: -2.5px;margin-bottom:2.5px;')}>{item.GovValIDNo} {item.NextFilter}</div>
                            <div>
                                <IonButton fill='clear' size="small" color="primary" style={styles('text-transform:capitalize;float:right;')} onClick={onUpdateIssuesConcern}>
                                    Update
                                </IonButton>
                            </div>
                        </div>
