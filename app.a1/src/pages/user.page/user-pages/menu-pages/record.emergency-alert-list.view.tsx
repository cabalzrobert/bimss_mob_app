...
export default class EmergencyAlertHistory extends React.Component implements OnPagerFocus { //OnSwapFocus, 
...
export class ViewHolder extends React.Component {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        console.log(filter);
        this.subs.s1 = rest.post('emergency/list', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
                                ...
                                <IonCard style={styles('--background:rgba(0, 6, 69, 0.1);')}>
                                    <IonCardContent>
                                        <div className={classNames('list-item', { 'bg': (!item.NextFilter || item.IsBlocked), '': !item.NextFilter, 'border-red': item.IsBlocked })} style={styles('')}>
                                            {!item.NextFilter ? <Item item={item} accntype={filter} onClick={() => /*filter.isGroup ? "" :*/ /*this.hViewProfile(item)*/ this.Open('https://maps.google.com/?q='+item.GeolocationLat + ',' + item.GeolocationLong) /*window.open(item.MemoURL)*/} onUpdateProfile={() => /*filter.isGroup ? "" :*/ this.hItem(item)} /> : <MoreItem item={item} onClick={() => this.hLoadMore(item)} />}
                                        </div>
                                    </IonCardContent>
                                </IonCard>
                                )}
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            <div style={styles('font-size: 12pt;font-weight: bold')}>{item.EmgyType} {item.NextFilter}</div>
                            <div style={styles('font-size: 10pt;margin-top: -2.5px')}>Date : {item.AlertDate} </div>
                            <div style={styles('font-size: 10pt;margin-top: -2.5px')}>Status : {item.Closed_TypeName} </div>
                            <div style={styles('font-size: 10pt;margin-top: -2.5px')}>Latitude : {item.GeolocationLat} </div>
                            <div style={styles('font-size: 10pt;margin-top: -2.5px')}>Longitude : {item.GeolocationLong} </div>
                        </div>
                    </div>
                </div>
