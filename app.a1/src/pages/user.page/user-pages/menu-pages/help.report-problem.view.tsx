...
import { filter, imagesOutline } from 'ionicons/icons';
...
export default class ReportProblemView extends React.Component {
...
export class ViewHolder extends React.Component {
    ...
    private performSubmit() {
        rest.post('concern/report/problem/closed', this.input).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.input);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({item, component:ReportProblemView, IsUpdate:!!this.input.isEdit,}));
            }
            ...
            <Layout full>
                <Layout>
                    <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-4 p-0 vertical toolbar-parent" onClick={this.hBackButton}>Issues Concern</div>
                        <div className="col-7 p-0 toolbar-title vertical"><div><TextFit text="Issues and Concern" /></div></div>
                    </div>
                </Layout>
                <Layout auto>
                    <IonContent scrollY>
                        <div className="row m-0 bootstrap-form old">
                            <div className='col-12 p-0 form-container' style={styles('width: 100%;background-color: #1d2c65;')}>
                                <form className={classNames('needs-validation', { 'form-invalid': !input.IsValid })} noValidate onSubmit={(ev) => ev.preventDefault()}>
                                    ...
                                    <div className={classNames({ 'input-invalid': !!msg.CorrectiveAction })} style={styles('position:relative;margin-top:10px')} >
                                        <IonItem lines="none" className='input-error' style={styles('--highlight-background:unset')}>
                                            <IonLabel position="stacked" style={styles('font-weight:bold;margin-top:-7.5px')}>Corrective Action</IonLabel>
                                            <Input ion node={(handle) => <>
                                                <IonTextarea rows={10} placeholder="Corrective Action Details"
                                                    value={input.CorrectiveAction} {...handle({ onChange: (ev => this.hState(ev, 'CorrectiveAction')) })} />
                                            </>} />
                                        </IonItem>
                                        <div className="invalid-tooltip" style={styles('top:98%')}>{msg.CorrectiveAction}</div>
                                    </div>
