...
export default class LeaderDirectAddMember extends React.Component {
...
export class ViewHolder extends React.Component {
    ...
    willMount = (didMount = true) => {
        ...
        this.subs.u = jUserModify(async () => {
            const u = await jUser();
            Object.rcopy(input, {
                SiteLeader: (u.isGroup) ? '' : u.USR_ID,
                SiteLeaderName: (u.isGroup) ? '' : u.FLL_NM,
                Region: u.LOC_REG,
                Province: u.LOC_PROV,
    ...
    setType(type: any) {
        ...
        this.input.isAdd = true;
        this.input.ise = false;
    ...
    private performSubmit() {
        rest.post('siteleader/member/new', this.input).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                ...
                return Alert.showSuccessMessage(res.Message,
                    () => this.pager.back({ item, component: LeaderDirectAddMember, IsUpdate: !!this.prop.IsUpdate, }));
            }
                                                        ...
                                                        <div className={classNames({ 'input-invalid': !!msg.Region })} style={styles('position:relative')}
                                                            ...
                                                            <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                                                ...
                                                                <Input ion="popup" node={(handle) => <>
                                                                    <IonInput hidden></IonInput>
                                                                    <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                                        value={input.Region} {...handle({ onChange: (ev) => this.hState(ev, 'Region') })} disabled={true} > {/* [(ngModel)]="filter.Method" (ionChange)="handleChangedPayMethod()"*/}
                                                        ...
                                                        <div className={classNames({ 'input-invalid': !!msg.Province })} style={styles('position:relative')}
                                                            ...
                                                            <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                                                ...
                                                                <Input ion="popup" node={(handle) => <>
                                                                    <IonInput hidden></IonInput>
                                                                    <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                                        value={input.Province} {...handle({ onChange: (ev) => this.hState(ev, 'Province') })} disabled={true} > {/* [(ngModel)]="filter.Method" (ionChange)="handleChangedPayMethod()"*/}
                                                        ...
                                                        <div className={classNames({ 'input-invalid': !!msg.Municipality })} style={styles('position:relative')}
                                                            ...
                                                            <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                                                ...
                                                                <Input ion="popup" node={(handle) => <>
                                                                    <IonInput hidden></IonInput>
                                                                    <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                                        value={input.Municipality} {...handle({ onChange: (ev) => this.hState(ev, 'Municipality') })} disabled={true} > {/* [(ngModel)]="filter.Method" (ionChange)="handleChangedPayMethod()"*/}
                                                        ...
                                                        <div className={classNames({ 'input-invalid': !!msg.Barangay })} style={styles('position:relative')}
                                                            ...
                                                            <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                                                ...
                                                                <Input ion="popup" node={(handle) => <>
                                                                    <IonInput hidden></IonInput>
                                                                    <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                                        value={input.Barangay} {...handle({ onChange: (ev) => this.hState(ev, 'Barangay') })} disabled={true} > {/* [(ngModel)]="filter.Method" (ionChange)="handleChangedPayMethod()"*/}
