...
export default class RequestBrgyOtherDocumentView extends React.Component {
    setType = (type: any, ClearanceNo: any, ControlNo: any) => (this.holder.setType(type, ClearanceNo, ControlNo), this);
...
export class ViewHolder extends React.Component {
    ...
    setForm(input: any) {
        ...
        this.subs.upt = timeout(() => {
            ...
            Object.rcopy(this.brgyrequest, {
                ...
                TemplateTypeNM: input.TemplateTypeNM,
                TemplateID: input.TemplateID,
                TemplateNM: input.TemplateNM,
    ...
    setType(type: any, ClearanceNo: any, ControlNo: any) {
        this.input.isAdd = type;
        this.input.ClearanceNo = ClearanceNo;
        this.input.ControlNo = ControlNo;
    ...
    private performSubmit() {
        rest.post('reqotherdocument/request', this.brgyrequest).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.brgyrequest);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({ item, component: RequestBrgyOtherDocumentView, IsUpdate: !!this.brgyrequest.IsUpdate, }));
            }
    ...
    private isValidEntries(): boolean {
        var isValid = true;
        if (!this.brgyrequest.TemplateTypeID) {
            this.msg.Subject = 'Please Select Type'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
        if (!this.brgyrequest.TemplateID) {
            this.msg.Subject = 'Please Select Document'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
    ...
    hTemplateType = async () => {
        ...
        const modal = await TemplateTypeSelectPopUp.modal("Type", { Type: 2, ID: this.brgyrequest.TemplateID });
        await modal.present();
        const brgyrequest = this.brgyrequest;
        const res = await modal.onDidDismiss();
        const data = (res || mtObj).data;
        if (!data) return;
        this.brgyrequest.TemplateTypeID = data.TemplateTypeID;
        this.brgyrequest.TemplateTypeNM = data.TemplateTypeNM;
        this.setState({ brgyrequest: this.brgyrequest });
    }
    hTemplateDoc = async () => {
        //if (!this.input.isAdd && this.input.STAT != "Open") return;
        // if (!this.input.Barangay)
        ...
        const modal = await TemplateDocSelectPopUp.modal("Document", { Type: 2, ID: this.brgyrequest.TemplateTypeID });
        await modal.present();
        const brgyrequest = this.brgyrequest;
        const res = await modal.onDidDismiss();
        const data = (res || mtObj).data;
        if (!data) return;
        this.brgyrequest.TemplateID = data.TemplateID;
        this.brgyrequest.TemplateNM = data.TemplateNM;
        this.setState({ brgyrequest: this.brgyrequest });
    }
            ...
            <Layout full>
                <Layout>
                    <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-4 p-0 vertical toolbar-parentx" onClick={this.hBackButton}>Brgy Other Document</div>
                        <div className="col-7 p-0 toolbar-title vertical"><div><TextFit text="Brgy Other Document" /></div></div>
                    </div>
                </Layout>
                <Layout auto>
                    <IonContent scrollY style={styles('background-color: #1d2c65;')}>
                            ...
                            <div className='col-12 p-0 form-container' style={styles('width: 100%;')}>
                                <form className={classNames('needs-validation', { 'form-invalid': !input.IsValid })} noValidate onSubmit={(ev) => ev.preventDefault()}>
                                    ...
                                    <div className={classNames({ 'input-invalid': !!msg.TemplateTypeID })} style={styles('position:relative')}
                                        ...
                                        <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            ...
                                            <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                value={this.brgyrequest.TemplateTypeID} onIonChange={(ev) => this.hState(ev, 'TemplateTypeID')}>
                                                <IonSelectOption value={this.brgyrequest.TemplateTypeID}>{this.brgyrequest.TemplateTypeNM}</IonSelectOption>
                                            </IonSelect>
                                        </IonItem>
                                    ...
                                    <div className={classNames({ 'input-invalid': !!msg.TemplateID })} style={styles('position:relative')}
                                        ...
                                        <IonItem className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            <IonLabel position="floating" style={styles('font-weight:bold;margin-top:-7.5px')}><span style={styles('color:red')}>*</span>Document</IonLabel>
                                            <IonInput hidden></IonInput>
                                            <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:40px')}
                                                value={this.brgyrequest.TemplateID} onIonChange={(ev) => this.hState(ev, 'TemplateID')}>
                                                <IonSelectOption value={this.brgyrequest.TemplateID}>{this.brgyrequest.TemplateNM}</IonSelectOption>
                                            </IonSelect>
                                        </IonItem>
...
class TemplateTypeSelectPopUp extends React.Component<{ modal: Function, Title: string, Opts: any }> implements OnDidBackdrop {
    private static cache: any = {};
    state: any = { inner: { height: (window.innerHeight + 'px') } }
    constructor(props: any) {
        super(props);
        ...
        this.prop.IsDevice = !device.isBrowser;
        this.filter.CacheID = (this.filter.Type + '-' + (this.filter.ID || '0'));
        const cache = TemplateTypeSelectPopUp.cache[this.filter.CacheID] || null;
        if (!!cache) {
            this.list = cache.List;
            this.prop.IsEmpty = (this.list.length < 1);
            this.prop.IsFiltering = this.prop.IsEmpty;
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        if (this.subs.s1) this.subs.s1.unsubscribe();
        console.log(this.filter);
        this.subs.s1 = rest.post('templatetype', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            this.prop.IsFiltering = false;
            if (res.Status != 'error') {
    ...
    static modal = async (title: string, opts: any) => {
        var modal: any;
        var stack = await Stack.push(<>
            <Modal className="modal-adjustment full" ref={(ref) => modal = ref} content={<TemplateTypeSelectPopUp modal={() => modal} Title={title} Opts={opts} />} />
        </>);
        setTimeout(async () => (await modal.onDidDismiss(), stack.pop()));
        return modal;
    }
...
class TemplateDocSelectPopUp extends React.Component<{ modal: Function, Title: string, Opts: any }> implements OnDidBackdrop {
    private static cache: any = {};
    state: any = { inner: { height: (window.innerHeight + 'px') } }
    constructor(props: any) {
        super(props);
        ...
        this.prop.IsDevice = !device.isBrowser;
        this.filter.CacheID = (this.filter.Type + '-' + (this.filter.ID || '0'));
        const cache = TemplateDocSelectPopUp.cache[this.filter.CacheID] || null;
        if (!!cache) {
            this.list = cache.List;
            this.prop.IsEmpty = (this.list.length < 1);
            this.prop.IsFiltering = this.prop.IsEmpty;
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        console.log(this.filter);
        this.subs.s1 = rest.post('templatedoc?templatetypeid=' + this.filter.ID, Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            this.prop.IsFiltering = false;
            if (res.Status != 'error') {
    ...
    static modal = async (title: string, opts: any) => {
        var modal: any;
        var stack = await Stack.push(<>
            <Modal className="modal-adjustment full" ref={(ref) => modal = ref} content={<TemplateDocSelectPopUp modal={() => modal} Title={title} Opts={opts} />} />
        </>);
        setTimeout(async () => (await modal.onDidDismiss(), stack.pop()));
        return modal;
    }
