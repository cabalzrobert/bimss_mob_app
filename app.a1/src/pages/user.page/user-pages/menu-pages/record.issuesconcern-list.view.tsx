...
export default class IssuesConcerHistory extends React.Component implements OnPagerFocus { //OnSwapFocus, 
...
export class ViewHolder extends React.Component {
    ...
    hMemberOpt = (ev: any) => {
        ...
        this.pager.next(ReportProblemView, ({ instance }: { instance: ReportProblemView }) => instance?.setType(this.filter.isAdd));
    }
    hLeaderOpt = (ev: any) => {
        //console.log(this.filter);
            ...
            <Layout full>
                <Layout>
                    <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-7 p-0 toolbar-title vertical"><div><TextFit text="Issues and Concern History" /></div></div>
                    </div>
                </Layout>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                            ...
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Trans. No.: {item.TRN_NO} {item.NextFilter}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Sitio: {item.SitioName}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Ticket No.: {item.TCKT_NO}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Subject: {item.SBJCT}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Details.: {item.BODY}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Corrective Action: {item.COR_ACTION}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Corrective Date: {item.FXD_TRN_TS}</div>
