...
export default class RequestBrgyClearanceView extends React.Component {
...
export class ViewHolder extends React.Component {
    ...
    private performSubmit() {
        rest.post('reqbrgyclearance/request', this.brgyrequest).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.brgyrequest);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({ item, component: RequestBrgyClearanceView, IsUpdate: !!this.brgyrequest.IsUpdate, }));
            }
    ...
    private isValidEntries(): boolean {
        ...
        console.log(this.brgyrequest.PurposeID);
        if (!this.brgyrequest.PurposeID) {
            this.msg.Subject = 'Please fill up purpose'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
    ...
    hTypeofClearance = async () => {
        ...
        const modal = await TypeofClearanceSelectPopUp.modal("Type of Clearance", { Type: 2, ID: this.brgyrequest.TypeofClearanceNM });
        ...
        this.brgyrequest.TypeofClearance = data.TypeofClearance;
        this.brgyrequest.TypeofClearanceNM = data.TypeofClearanceNM;
    ...
    hPurpose = async () => {
        ...
        const modal = await PurposeSelectPopUp.modal("Purpose", { Type: 2, ID: this.brgyrequest.Purpose });
        //const modal = await SitioSelectPopUp.modal('Sitio', { Type: 2, ID: this.input.Barangay });
        await modal.present();
        const brgyrequest = this.brgyrequest;
        const res = await modal.onDidDismiss();
        ...
        // input.Sitio = data.SIT_ID;
        // input.SitioName = data.SIT_NM;
        this.brgyrequest.PurposeID = data.PurposeID;
        this.brgyrequest.PurposeNM = data.PurposeNM;
        this.setState({ brgyrequest: this.brgyrequest });
    }
            ...
            <Layout full>
                <Layout>
                    {/* <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-7 p-0 vertical" style={styles('align-items: end;color: black;font: message-box;')}><div><TextFit text="Brgy Clearance" /></div></div>
                    </div> */}
                    <IonHeader>
                        ...
                        <div className="row m-0 bootstrap-form old">
                            <div className='col-12 p-0 form-container' style={styles('width: 100%;')}>
                                <form className={classNames('needs-validation', { 'form-invalid': !input.IsValid })} noValidate onSubmit={(ev) => ev.preventDefault()} style={styles('padding:10px;')}>
                                    ...
                                    <div className={classNames({ 'input-invalid': !!msg.TypeofClearance })} style={styles('position:relative')}
                                        onClick={this.hTypeofClearance}>
                                            <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Type of Clearance</IonLabel>
                                        <IonItem lines='none' className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            ...
                                            <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:50px;color:#737373;font-weight:bold;')}
                                                value={this.brgyrequest.TypeofClearance} onIonChange={(ev) => this.hState(ev, 'TypeofClearance')}>
                                                <IonSelectOption value={this.brgyrequest.TypeofClearance}>{this.brgyrequest.TypeofClearanceNM}</IonSelectOption>
                                            </IonSelect>
                                        </IonItem>
                                        <div className="invalid-tooltip">{msg.TypeofClearance}</div>
                                        <div style={styles('position:absolute;top:0;width:100%;height:100%;z-index:1')}>&nbsp;</div>
                                    </div>

                                    <div className={classNames({ 'input-invalid': !!msg.PurposeID })} style={styles('position:relative;margin-top:7.5px;')}
                                        onClick={this.hPurpose}>
                                            <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Purpose</IonLabel>
                                        <IonItem lines='none' className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            <IonLabel hidden position='stacked'></IonLabel>
                                            <IonInput hidden></IonInput>
                                            <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:50px;color:#737373;font-weight:bold;')}
                                                value={this.brgyrequest.PurposeID} onIonChange={(ev) => this.hState(ev, 'PurposeId')}>
                                                <IonSelectOption value={this.brgyrequest.PurposeID}>{this.brgyrequest.PurposeNM}</IonSelectOption>
                                            </IonSelect>
                                        </IonItem>
                                        <div className="invalid-tooltip">{msg.PurposeID}</div>
                                        <div style={styles('position:absolute;top:0;width:100%;height:100%;z-index:1')}>&nbsp;</div>
                                    </div>


                                    {/* <div className="horizontal" style={styles('margin-top:5px;height:auto')}>
                                        <IonButton className="btn-green" style={styles('width:200px')} onClick={this.hConfirm}>Submit</IonButton>
                                    </div> */}
                                    <div className="horizontal" style={styles('margin-top:10%;height:40px')}>
