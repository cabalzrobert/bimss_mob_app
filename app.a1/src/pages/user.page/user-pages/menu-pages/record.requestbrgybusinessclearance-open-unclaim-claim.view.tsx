...
import { addCircleOutline, addOutline, arrowBackOutline, filter, searchOutline } from 'ionicons/icons';
...
import RequestBrgyBusinessClearanceView from './brgybusinessclearance.new.view';
import RequestBrgyBusinessClearancePopUp from '../../../+app/modal/requestbrgybusinessclearance.view.pop';

const { Object }: any = window;

export default class RequestBarangayBusinessClearacneAppHistory extends React.Component implements OnPagerFocus {
    static activeTab: any = null;
    onPagerFocus = (data: any) => (RequestBarangayBusinessClearacneAppHistory.activeTab && RequestBarangayBusinessClearacneAppHistory.activeTab.onPagerFocus(data));
...
export class ViewHolder extends React.Component {
    ...
    willMount = (didMount = true) => {
        ...
        const page = (this.page = { ParentTitle: '', PageTitle: 'Brgy. Business Clearance' });
        ...
        this.setState({ tabs: false, page, prop });
    }
    didMount = () => {
                ...
                <Layout>
                    ...
                    <IonHeader>
                        ...
                        <div style={styles('height:70px;')}>
                            ...
                            //         <IonMenuButton />
                            //         <IonTitle>Brgy Business Permit</IonTitle>
                            //     </IonButtons>
                            // </IonToolbar>
                            }
                    </IonHeader>
...
class PowerApTabsView extends React.Component {
    ...
    tabs: any = [
        ...
        { name: 'Process', component: ClaimTab2View },
    ...
    onSwapChange = () => {
        return timeout(() => {
            RequestBarangayBusinessClearacneAppHistory.activeTab = this.swapper.currentComponent.instance;
        });
...
class PageFilterOpenView extends React.Component<{ status?: (undefined | '' | '1' | '2' | '3') }> {
    ...
    componentWillMount = () => {
        ...
        filter.IsCompleted = (props.status == '');
        this.subs.u = jUserModify(async () => {
            const u = await jUser();
            Object.rcopy(filter, {
                Userid: u.USR_ID,
    ...
    hItem = async (item: any) => {
        ...
        this.pager.next(RequestBrgyBusinessClearanceView, ({ instance }: { instance: RequestBrgyBusinessClearanceView }) => {
            instance?.setType(item.isAdd, item.BusinessClearanceID, item.ControlNo).setForm(item);
        });
    }
    ...
    async popBrgyClearanceOpenPopUp(subscriber: any) {
        const modal = await RequestBrgyBusinessClearancePopUp.modal(subscriber);
        await modal.present();
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        if (!this.subs) return;
        if (this.subs.s1) this.subs.s1.unsubscribe();
        this.subs.s1 = rest.post('reqbrgybusinessclearance', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
    ...
    hMemberOpt = (ev: any) => {
        ...
        this.pager.next(RequestBrgyBusinessClearanceView, ({ instance }: { instance: RequestBrgyBusinessClearanceView }) => instance?.setType(this.filter.isAdd, type, 'Request Business Clearance'));
    ...
    hAddItem = (ev: any) => {
        ...
        this.filter.STAT = "Open";
        var type = (ev.detail || mtObj).value;
        // this.pager.next(RequestBrgyBusinessClearanceView, ({ instance }: { instance: RequestBrgyBusinessClearanceView }) => instance?.setType(type, '', ''));
                    ...
                    <IonContent scrollY>
                        ...
                        <IonList lines='none'>
                            <div className="list wbg">
                                ...
                                <IonCard style={styles('--background:rgb(219, 221, 237, 0.3);')}>
                                    <IonCardContent>
                        ...
                        <IonList lines='none'>
                            <div className="list wbg filter_options-bg">
                                ...
                                <IonCard style={styles('--background:rgba(0, 6, 69, 0.1);')}>
                                    <IonCardContent>
                                        <div className={classNames('list-item', { 'bg': (!item.NextFilter || item.IsBlocked), 'bg-black': !item.NextFilter, 'border-red': item.IsBlocked })} style={styles('')}>
                                            {!item.NextFilter ? <RecievedItem item={item} accntype={filter} onClick={() => this.hViewProfile(item)} onUpdateIssuesConcern={() => this.hItem(item)} onViewAttachment={() => this.hViewAttachement(item)} /> : <MoreItem item={item} onClick={() => this.hLoadMore(item)} />}
...
class PageFilterPendingView extends React.Component<{ status?: (undefined | '' | '1' | '2' | '3') }> {
    ...
    hViewProfile = async (item: any) => {
        this.selectedItem = item;
        this.popBrgyBusinessClearanceOpenPopUp(item);
    }

    async popBrgyBusinessClearanceOpenPopUp(subscriber: any) {
        const modal = await RequestBrgyBusinessClearancePopUp.modal(subscriber);
        await modal.present();
        const { data: { IsDone } = mtObj } = await modal.onDidDismiss();
        if (!IsDone) return;
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            <div style={styles('font-size: 14pt;font-weight: bold')}>{item.BusinessNM}</div>
                            <div style={styles('font-size: 12pt;;font-weight: bold;margin-top: -2.5px')}>{item.OwnerNM}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Control. No.: {item.ControlNo} {item.NextFilter}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Request: Business Clearance</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Application Date: {item.ApplicationDate}</div>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                ...
                                <div className="btn-update" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onUpdateIssuesConcern}>Update Brgy Business Clearance</div>&nbsp; */}
                                <IonButton size='small' onClick={onClick}>Details</IonButton>
                                <IonButton color='success' size='small' onClick={onUpdateIssuesConcern}>Update</IonButton>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            ...
                            <div style={styles('font-size: 14pt;font-weight: bold;margin-top: -2.5px')}>{item.BusinessNM}</div>
                            <div style={styles('font-size: 12pt;font-weight: bold')}>{item.OwnerNM}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Control. No.: {item.ControlNo} {item.NextFilter}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Request: Business Clearance</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Application Date: {item.ApplicationDate}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Appointment Date: {item.AppointmentDate}</div>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                {/* <div className="btn-primary" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onClick}>View Req. Doc.</div>&nbsp;
                                <div className="btn-update" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onUpdateIssuesConcern}>Update Brgy Business Clearance</div>&nbsp; */}
...
const PendingItem: React.FC<{ item: any, accntype: any, onClick?: React.MouseEventHandler, onProccessIssuesConcern?: React.MouseEventHandler, onViewAttachment?: React.MouseEventHandler, }> = ({ item, accntype, onClick, onProccessIssuesConcern, onViewAttachment }) => {
    ...
    return (<>
        <div className="row m-0 details ion-activatable" style={styles('padding:5px')}>
            <IonRippleEffect />
            <div className="col-12 p-0 row m-0" style={styles('position:relative;width:100%;color:#737373')}>
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            ...
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Request: Business Clearance</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Application Date: {item.ApplicationDate}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Status: {(item.Release) ? "Release" : "For Releasing"}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Issued By: {(item.Release) ? item.ReleasedBy : ""}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Issued Date: {(item.Release) ? item.DateRelease : ""}</div>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                {/* <div className="btn-primary" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onClick}>View Req. Doc.</div>&nbsp; */}
                                <IonButton size='small' onClick={onClick}>Details</IonButton>
                            </div>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            ...
                            <div style={styles('font-size: 14pt;font-weight: bold;margin-top: -2.5px')}>{item.BusinessNM}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')}>{item.OwnerNM}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Control. No.: {item.ControlNo} {item.NextFilter}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Request: Business Clearance</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')}>Application Date: {item.ApplicationDate}</div>

                            <div style={styles('font-size: 11pt;margin-top: -2.5px')} >Status: Cancelled</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')} >Cancelled by: {item.CancelledBy}</div>
                            <div style={styles('font-size: 11pt;margin-top: -2.5px')} >Date Cancelled: {item.CancelledDate}</div>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                {/* <div className="btn-primary" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onClick}>View Req. Doc.</div>&nbsp;
                                <div className="btn-primary" style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onOpen}>Open Req. Doc.</div>&nbsp; */}
                                {/* <IonButton size='small' onClick={onClick}>Details</IonButton> */}
                                <IonButton size='small' onClick={onOpen}>Open</IonButton>
                            </div>
