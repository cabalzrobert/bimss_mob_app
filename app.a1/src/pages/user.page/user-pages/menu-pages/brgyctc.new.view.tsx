...
export default class RequestBrgyCTCView extends React.Component {
...
export class ViewHolder extends React.Component {
    ...
    private performSubmit() {
        rest.post((this.input.isEdit) ? 'reqclearance/edit' : 'reqclearance/new', this.input).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.input);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({ item, component: RequestBrgyCTCView, IsUpdate: !!this.input.isEdit, }));
            }
    ...
    render() {
        ...
        this.input.Category_Document = '4';
            ...
            <Layout full>
                <Layout>
                    <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-7 p-0 toolbar-title vertical"><div><TextFit text="Brgy Clearance" /></div></div>
                    </div>
                </Layout>
                <Layout auto>
                    <IonContent scrollY>
