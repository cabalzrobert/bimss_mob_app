...
export default class RequestBrgyBusinessClearanceView extends React.Component {
    setType = (type: any, BusinessClearanceID: any, ControlNo: any) => (this.holder.setType(type, BusinessClearanceID, ControlNo), this);
...
export class ViewHolder extends React.Component {
    ...
    setForm(input: any) {
        ...
        this.subs.upt = timeout(() => {
            ...
            Object.rcopy(this.brgyrequest, {
                ...
                BusinessClearanceID: input.BusinessClearanceID,
                BusinessID: input.BusinessID,
                BusinessNM: input.BusinessNM,
    ...
    setType(type: any, BusinessClearanceID: any, ControlNo: any) {
        Object.rcopy(this.brgyrequest, {
            PL_ID: this.input.PL_ID,
            PGRP_ID: this.input.PGRP_ID,
            Birthdate: this.input.BirthDate,
            MobileNumber: this.input.MobileNumber,
            ClearanceNo: this.input.ClearanceNo,
            ControlNo: this.input.ControlNo,
            BusinessClearanceID: this.input.BusinessClearanceID,
            BusinessID: this.input.BusinessID,
            BusinessNM: this.input.BusinessNM,
            IsUpdate: this.prop.IsUpdate,
        });
    ...
    private performSubmit() {
        rest.post('reqbrgybusinessclearance/request', this.brgyrequest).subscribe(async (res: any) => {
            if (res.Status == 'ok') {
                const item = Object.rcopy(res.Content, this.brgyrequest);
                return Alert.showSuccessMessage(res.Message, () => this.pager.back({ item, component: RequestBrgyBusinessClearanceView, IsUpdate: !!this.brgyrequest.IsUpdate, }));
            }
    ...
    private isValidEntries(): boolean {
        var isValid = true;
        if (!this.brgyrequest.BusinessID) {
            this.msg.Subject = 'Please Select Business'; //this.msg.Subject = 'Please fill up subject';
            isValid = false;
        }
    ...
    hTypeofClearance = async () => {
        const modal = await TypeofClearanceSelectPopUp.modal("Select Business Name", { Type: 2, ID: this.brgyrequest.TypeofClearanceNM });
        await modal.present();
        const brgyrequest = this.brgyrequest;
        const res = await modal.onDidDismiss();
        const data = (res || mtObj).data;
        if (!data) return;
        this.brgyrequest.BusinessID = data.BusinessID;
        this.brgyrequest.BusinessNM = data.BusinessName;
            ...
            <Layout full>
                <Layout>
                    {/* <div className="row m-0 toolbar-panel">
                        ...
                        <div className="col-7 p-0 vertical" style={styles('align-items: end;color: black;font: message-box;')}><div><TextFit text="Biz Clearance" /></div></div>
                    </div> */}
                    <IonHeader>
                ...
                <Layout auto>
                    <br/>
                    <IonContent scrollY style={styles('background-color: #ffffff;')}>
                        <div className="row m-0 bootstrap-form old">
                            <div className='col-12 p-0 form-container' style={styles('width: 100%;')}>
                                <form className={classNames('needs-validation', { 'form-invalid': !input.IsValid })} noValidate onSubmit={(ev) => ev.preventDefault()} style={styles('padding:10px;')}>
                                    ...
                                    <div className={classNames({ 'input-invalid': !!msg.BusinessID })} style={styles('position:relative')}
                                        onClick={this.hTypeofClearance}>
                                            <IonLabel position="fixed" style={styles('font-weight:bold;margin-top:-7.5px;margin-left:12px;')}><span style={styles('color:red')}>*</span>Business Name</IonLabel>
                                        <IonItem lines='none' className='input-error' style={styles('--highlight-background:unset;--min-height:40px;margin-top:2.5px')}>
                                            ...
                                            <IonSelect interface="action-sheet" cancelText="Dismiss" style={styles('height:50px;color:#737373;font-weight:bold;')}
                                                value={this.brgyrequest.BusinessID} onIonChange={(ev) => this.hState(ev, 'BusinessID')}>
                                                <IonSelectOption value={this.brgyrequest.BusinessID}>{this.brgyrequest.BusinessNM}</IonSelectOption>
                                            </IonSelect>
                                        </IonItem>
                                        <div className="invalid-tooltip">{msg.BusinessID}</div>
...
class TypeofClearanceSelectPopUp extends React.Component<{ modal: Function, Title: string, Opts: any }> implements OnDidBackdrop {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('business', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
            this.prop.IsFiltering = false;
            if (res.Status != 'error') {
                if (filter.IsReset) this.list = res.biz.map((o: any) => this.listDetails(o));
                else res.biz.forEach((o: any) => this.list.push(this.listDetails(o)));
                ...
                <Layout full style={styles('height:calc(100% - 30px)')}>
                    <Layout>
                        <IonSearchbar value={filter.Search} onIonChange={(ev) => (this.hFState(ev, 'Search'), this.hFilter())}
                            placeholder="Search Business Name" />{/*onIonChange={e => setSearchText(e.detail.value!)} */}
                    </Layout>
                    <Layout auto>
