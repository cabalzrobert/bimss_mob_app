...
export default class EmployementAppHistory extends React.Component implements OnPagerFocus { //OnSwapFocus, 
...
export class ViewHolder extends React.Component {
    ...
    hAddItem = (ev: any) => {
        ...
        this.pager.next(AccountEmploymentHistoryView, ({ instance }: { instance: AccountEmploymentHistoryView }) => instance?.setType(this.filter.isAdd));
    }
    // hMemberOpt = (ev: any) => {
    //     var type = (ev.detail || mtObj).value;
    //     if (!type) return;
    ...
    hItem = async (item: any) => {
        ...
        this.pager.next(AccountEmploymentHistoryView, ({ instance }: { instance: AccountEmploymentHistoryView }) => {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        ...
        this.subs.s1 = rest.post('employment/history', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#737373;')}>
                            ...
                            <div style={styles('font-size: 14pt;font-weight: bold')}>{item.Company}</div>
                            <div style={styles('font-size: 12pt;')}>{item.CompanyAddress}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')}>From: {item.RenderedFrom} {item.NextFilter}</div>
                            <div style={styles('font-size: 12pt;margin-top: -2.5px')}>To: {item.RenderedTo}</div>
                        ...
                        <div className="horizontal" style={styles('justify-content: left;')}>
                            ...
                            <IonButton size='small' style={styles('text-transform:capitalize;')} onClick={onUpdateIssuesConcern}>Update</IonButton>
                        </div>
