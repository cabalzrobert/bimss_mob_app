import { IonButtons, IonContent, IonFab, IonFabButton, IonHeader, IonIcon, IonInput, IonItem, IonLabel, IonList, IonMenuButton, IonRefresher, IonRefresherContent, IonRippleEffect, IonSelect, IonSelectOption, IonSpinner, IonTitle, IonToolbar } from '@ionic/react';
...
export default class BlotterComplaintSummonAppHistory extends React.Component implements OnPagerFocus {
    ...
    //};
    onPagerFocus = (data: any) => (BlotterComplaintSummonAppHistory.activeTab && BlotterComplaintSummonAppHistory.activeTab.onPagerFocus(data));
...
export class ViewHolder extends React.Component {
    ...
    subs: any = {};
    willMount = () => {
        const page = (this.page = { ParentTitle: '', PageTitle: 'Blotter' });
...
class PowerApTabsView extends React.Component {
    ...
    tabs: any = [
        { name: 'Blotter', component: OpenTab1View },
        { name: 'Complaint', component: PendingTab2View },
        { name: 'Close', component: ClosedTab3View },
        { name: 'Canceled', component: ClosedTab4View },
    ];
    ...
    onSwapChange = () => {
        return timeout(() => {
            BlotterComplaintSummonAppHistory.activeTab = this.swapper.currentComponent.instance;
        });
...
class ClosedTab3View extends React.Component implements OnSwapFocus, OnSwapLeave {
...
}class ClosedTab4View extends React.Component implements OnSwapFocus, OnSwapLeave {
    view = ({} as PageFilterClosedView);
    onPagerFocus = (data: any) => null;
    onSwapFocus() { this.view.resumeRequest(); }
    onSwapLeave() { this.view.pauseRequest(); }
    render() {
        return (<>
            <PageFilterCanceledView ref={(ref: any) => this.view = ref} status='3' />
        </>);
    }
}
...
class PageFilterOpenView extends React.Component<{ status?: (undefined | '0' | '1' | '2' | '3') }> {
    ...
    hItem = async (item: any) => {
        ...
        this.pager.next(BlotterComplaintSummonAttachmentView, ({ instance }: { instance: BlotterComplaintSummonAttachmentView }) => {
    ...
    private performRequest(filter: any, callback: Function = mtCb) {
        if (!this.subs) return;
        if (this.subs.s1) this.subs.s1.unsubscribe();
        this.subs.s1 = rest.post('blotter/complaint/history', Object.rcopy(filter, this.filter)).subscribe(async (res: any) => {
        ...
        return (<>
            ...
            <div style={styles('visibility:hidden;height:0px;width:0px')}>
                <IonSelect ref={this.elIonSelect} interface="action-sheet"
                    ...
                    <IonSelectOption value='6'>Blotter</IonSelectOption>
                </IonSelect>
                {/* 
...
}class PageFilterCanceledView extends React.Component<{ status?: (undefined | '0' | '1' | '2' | '3') }> {
    state: any = {};
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                            <div className="horizontal" style={styles('justify-content: left;')}>
                                ...
                                <div className="btn-update" hidden={(item.Status_Type == "0" ) ? true : false} style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onUpdateIssuesConcern}>Update Issues</div>&nbsp;
                                <div className="btn-primary" hidden={(item.TotalAttachment == 0) ? false : true} style={styles('min-width: 20%;padding: 5px;font-size: 8px;height: 10%;')} onClick={onViewAttachment}>View Attachment</div>&nbsp;
                            </div>

                            <div style={styles('font-weight: bold')}>Respondent: {item.RespondentName}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Issues: {item.Issue} {item.NextFilter}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Complaint Type: {item.ComplaintTypeName}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Place.: {item.IncidentPlaceName}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Incident Date.: {item.IncidentDate}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Status.: {item.Status_Type_Name}</div>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                            ...
                            <div style={styles('font-weight: bold')}>Concern Citizen: {item.Fullname}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Trans. No.: {item.TransactionNo} {item.NextFilter}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Sitio: {item.SitioName}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Ticket No.: {item.TicketNo}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Subject: {item.Subject}</div>
                            ...
                            {/* <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Corrective Action: {item.CorrectiveAction}</div> */}
                            {/* <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Corrective Date: {item.ActionDate}</div> */}
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Status: {item.STAT}</div>
                        </div>
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                ...
                <div className="layout-horizontal">
                    ...
                    <div className="auto">
                        <div className="vertical" style={styles('position:relative;color:#fefed1;')}>
                            ...
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Issued Date.: {item.IssuedDate}</div>
                            {/* <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Corrective Action: {item.CorrectiveAction}</div> */}
                            <div style={styles('font-size: 12px;margin-top: -2.5px')} hidden={(item.STAT == "Closed") ? false : true}>Closed Date: {item.ActionDate}</div>
                            <div style={styles('font-size: 12px;margin-top: -2.5px')}>Status: {item.STAT}</div>
                        </div>
                    </div>
                </div>
